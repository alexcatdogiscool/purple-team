...
int Application::encrypt(int key, int msg) {
    return msg ^ key;
}

int Application::decrypt(int key, int cipher) {
    return cipher ^ key;
}


void Application::encryptStr(int k, string msg, cipherStr& c) {
    c.length = msg.length();
...
char* Application::decryptStr(int k, cipherStr ciph, char* msg) {
